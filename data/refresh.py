...
# Cotizaciones argentinas, gratis y sin clave. Se usa el tipo tarjeta porque es
# el que pagarias comprando en el exterior: asi la comparacion es entre dos
# compras posibles, no contra un dolar teorico.
FUENTES_CAMBIO = [
...
def actualizar_cambio():
    """Guarda las cotizaciones en data/cambio.json. Si falla, no rompe nada:
    el catalogo simplemente no muestra la comparacion."""
    datos = {"fecha": HOY, "fuente": "dolarapi.com"}
    for clave, url in FUENTES_CAMBIO:
        try:
            crudo = json.loads(leer(url, 50_000))
        except Exception as e:
            print(f"   cotizacion {clave}: no se pudo leer ({str(e)[:40]})")
            continue
        ...
        if isinstance(valor, (int, float)):
            ...
            print(f'   cotizacion {clave}: {valor} ({datos[clave]["nombre"]})')
        time.sleep(0.5)
    if len(datos) > 2:
        CAMBIO.write_text(json.dumps(datos, ensure_ascii=False, indent=1), encoding="utf-8")
        print("   escribi data/cambio.json")
    return datos
